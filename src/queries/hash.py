...
hash_query = """
    ...
    SELECT "threatactor_count" AS ioc_type, COUNT(*) AS count FROM tls.edges
    WHERE edge_name = @threatactor_count
    AND to_ver = @ioc_value

    UNION ALL

    SELECT "campaign_count" AS ioc_type, COUNT(*) AS count FROM tls.edges
    WHERE edge_name = @campaign_count
    AND to_ver = @ioc_value

    UNION ALL

    SELECT "vuln_count" AS ioc_type, COUNT(*) AS count FROM tls.edges
    WHERE edge_name = @vuln_count
    AND from_ver = @ioc_value
