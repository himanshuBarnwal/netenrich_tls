...
ip_query = """
    ...
    SELECT "honeypots" AS ioc_type, MIN(edge_first_seen) as first_seen,MAX(edge_first_seen) as last_seen 
    FROM tls.edges WHERE edge_name = @honeypots AND from_ver = @ioc_value AND to_ver = 'honeypot' ;
    ...
    WITH search_graph_fqdn AS (
        SELECT edge_name, from_ver, to_ver,to_type, last_seen,edge_total_seen FROM (
            ...
            FROM tls.edges WHERE to_ver = @ioc_value AND edge_name = @fqdns
    ...
    SELECT "fqdns" AS ioc_type, from_ver AS key,MAX(last_seen) AS date, SUM(edge_total_seen) AS hits FROM search_graph_fqdn GROUP BY from_ver
